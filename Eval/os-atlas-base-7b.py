#!/usr/bin/env python3
"""Run OS-Atlas-Base-7B on ScreenSpot and log per-sample grounding results."""
...
def setup_logger(log_path: Path) -> logging.Logger:
    logger = logging.getLogger("screenspot")
...
def prediction_to_vis_box(parsed: Dict[str, Any], width: int, height: int) -> Optional[List[int]]:
    """Convert parsed prediction to a drawable bbox in pixel coordinates."""
...
def draw_prediction_on_image(
    ...
    """
    Draw predicted box on image with YELLOW color and save.
    Always use yellow, regardless of success/failure.
    """
    # 黄色 (R, G, B)
    YELLOW = (255, 255, 0)
...
def ensure_paths(args: argparse.Namespace) -> Tuple[Path, Path, Optional[Path]]:
    """Ensure output directories exist."""
    args.output_dir.mkdir(parents=True, exist_ok=True)
    log_file = args.output_dir / f"{args.run_name}.log"
    result_jsonl = args.output_dir / f"{args.run_name}_results.jsonl"
    ...
    return log_file, result_jsonl, vis_dir


def main() -> None:
    args = parse_args()
    log_file, result_jsonl, vis_dir = ensure_paths(args)
    ...
    with result_jsonl.open("w", encoding="utf-8") as result_f:
        for idx, item in enumerate(records, start=1):
            ...
            if not img_path.exists():
                ...
                result_line = json.dumps(sample_result, ensure_ascii=False)
